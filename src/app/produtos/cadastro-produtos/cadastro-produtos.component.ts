...
export class CadastroProdutosComponent extends BaseFormComponent implements OnInit {
  ...
  submit() {
    console.log (this.formulario.get('dataCadastro').value);

    this.produtosService.save(this.formulario.value).subscribe(
    error => console.error(error),
    () => console.log('request completo'),
    );
  // alert(`${this.formulario.get('categoria.codigo').value} cadastrado com sucesso!`);
    this.resetar();
  }
...
popularCampoQrcode() {
  ...
  if (serie != null && serie !== '') {
      ...
      this.formulario.get('resourceWebServicesQrcode')
      .setValue(`${webserviceqrcode.apiUrl}${environment.apiUrl}/produtos/${serie}`);
  }
}
